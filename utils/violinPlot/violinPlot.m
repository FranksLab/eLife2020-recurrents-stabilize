...
%                6: draw lines at the 2.5,50,97.5 percentiles (no mean)
...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    parserObj.addOptional('color',def.color);
        parserObj.addOptional('spport',def.spport);
...
%                 
                 mdh = myErrorbar(ah,m,opt.xValues+xOffset,[sem,NaN(size(sem))]);
                mh = plot(ah,m,opt.xValues+xOffset,'o','markerfacecolor','w','MarkerSize',markersz, 'markeredgecolor', 'k');
            end
            if opt.showMM == 5
%                 mh = plot(ah,m,opt.xValues+xOffset,'+r','Color','k','MarkerSize',markersz);
