function [ACC, ACCx, TypeStack, task, p] = Crossifier_liblinear_ntng(Catalog,BinSizes,PST,Permutations,samplesizelist,task,Type,specialparams)
...
statelist = ['a','k'];
[X,Y,n_bins] = CrossPopulator_ntng(Catalog, efds, BinSizes, PST, statelist); % if abs(BinSize-diff(PST))<BinSize there will be 1 bin.
...
for pm = 1:Permutations
    ...
    for ssz = 1:length(samplesizelist)
        ...
        Xs = SubsetPermuter_Cross(X, Y, samplesize, TypeStack, BinSizes, n_bins);
