...
figure(1)
printpos([500 500 200 200])
clf
colores = [0.2 0.2 0.2; 0.051 0.447 0.7294; 236/255 0/255 139/255; .7 .7 .7];

% peak histograms
% awake
subplot(1,2,1)

figure_boxplot({Pa(CSa), Pa(CSc)},{[];[]},[],[],'horizontal',colores([1,3],:));
ylim([-10 160])
box off;
set(gca,'XTick',[])
[h,p,~,stats] = ttest2(Pa(CSa), Pa(CSc));
title(num2str(p))
...
% awake
subplot(1,2,2)
figure_boxplot({La(CSa), La(CSc)},{[];[]},[],[],'horizontal',colores([1,3],:));
box off;
set(gca,'XTick',[])
[h,p,~,stats] = ttest2(La(CSa), La(CSc));
title(num2str(p))
ylim([-.05 .55])
